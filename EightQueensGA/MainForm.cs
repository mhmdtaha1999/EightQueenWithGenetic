...
namespace EightQueensGA
{
    public partial class MainForm : Form
    {
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            List<Chromosome> bestChromosome = new List<Chromosome>();
            if(chkPorgress.Checked)
                geneticAlgo.progress +=new Progress(updateProgress);
            progressBar1.Maximum = (int)txtGen.Value;
            progressBar1.Value = 0;
            geneticAlgo.DoMating(ref initPopulation, (int)txtGen.Value, (double)txtCrosProb.Value, (double)txtMutProb.Value, (double)txtElitismRate.Value, chartavg  ,ref bestChromosome);
            ...
            dgBestFitness.Rows.Clear();
            for(int i = 0; i < bestChromosome.Count; i++)
            {
                string sol = "| ";
                for (int j = 0;j < 8; j++)
                {
                    sol = sol + bestChromosome[i].genes[j] + " | ";
                }
                dgBestFitness.Rows.Add(new object[] {sol, bestChromosome[i].fitness });
            }
